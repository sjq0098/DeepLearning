...
class SODDataset(Dataset):
    ...
    def __init__(self, datapath: str, mode: str = "train", size: int = 352, split: str = None,
                 mask_subdir: str = "ground_truth_mask"):
        # `mode` controls behaviour (train -> augment, test -> native-res GT).
        # `split` controls which <name>.txt file to read; defaults to `mode`.
        # `mask_subdir` selects the GT folder — set to a gt_noisy_* dir for the
        # label-noise study (train on noisy GT, but always test on clean GT).
    ...
    def __getitem__(self, idx):
        ...
        mask = cv2.imread(os.path.join(self.datapath, self.mask_subdir, name + ".png"), 0).astype(np.float32)
...
def get_train_loader(datapath: str, batch_size: int = 32, num_workers: int = 4, split: str = "train",
                     mask_subdir: str = "ground_truth_mask"):
    dataset = SODDataset(datapath, mode="train", split=split, mask_subdir=mask_subdir)
    return DataLoader(
        dataset,
